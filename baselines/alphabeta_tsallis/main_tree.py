...
def experiment_on_datasets_single_tree(
    ...
    for dataset_id, dataset_name in datasets:
        ...
        for seed in seeds:
            ...
            logger.info(f"{dataset_name} {seed} - Best Params: {best_params}, Train Accuracy: {best_train_acc:.4f}, Test Accuracy: {test_acc:.4f}, Num Nodes: {num_nodes}")
            end = time.time()
        ...
        logger.info(f"{dataset_name} - Mean Test Accuracy: {mean_test_accuracy:.4f}, Std: {std_test_accuracy:.4f}, Mean Num Nodes: {mean_num_nodes:.2f}, Std Num Nodes: {std_num_nodes:.2f}, Mean Time: {mean_time:.4f}, Std Time: {std_time:.4f}")
    return results
...
if __name__ == "__main__":
    seeds = range(1, 6)
    alphas = [0.5]
    betas = [i + 1 for i in range(1)]
    results = experiment_on_datasets_single_tree(seeds, alphas=alphas, betas=betas)

    # Print final results
    logger.info("\nFinal Results:")
    for dataset, metrics in results.items():
        logger.info(f"{dataset} - Mean Test Accuracy: {metrics['mean_test_accuracy']:.4f}, Std: {metrics['std_test_accuracy']:.4f}, Mean Num Nodes: {metrics['mean_num_nodes']:.2f}, Std Num Nodes: {metrics['std_num_nodes']:.2f}, Mean Time: {metrics['mean_time']:.4f}, Std Time: {metrics['std_time']:.4f}")
