...
def experiment_on_datasets(seeds):
    ...
    for dataset_id, dataset_name in datasets:
        ...
        accuracies, model_sizes, execution_times = [], [], []
        for seed in seeds:
            accuracy, model_size, execution_time = run_dpdt_on_dataset(X, y, seed)
        ...
        print(
            f"{dataset_name} - Mean Model Size: {mean_model_size:.4f}, Std: {std_model_size:.4f}"
        )
        print(
            f"{dataset_name} - Mean Execution Time: {mean_execution_time:.4f} seconds, Std: {std_execution_time:.4f}"
        )
...
for dataset, metrics in results.items():
    ...
    print(
        f"{dataset} - Mean Model Size: {metrics['mean_model_size']:.4f}, Std: {metrics['std_model_size']:.4f}"
    )
    print(
        f"{dataset} - Mean Execution Time: {metrics['mean_execution_time']:.4f} seconds, Std: {metrics['std_execution_time']:.4f}"
    )
