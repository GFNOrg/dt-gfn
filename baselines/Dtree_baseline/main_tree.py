    ...
    for i, a in enumerate(alphas):
        for j, b in enumerate(betas):
            tree = DtreeDecisionTreeClassifier(
                max_depth=max_depth, min_samples_split=10, a=a, b=b, alpha=0.01
            )
...
def run_dtree_on_dataset(
    ...
    best_tree, best_params, best_train_acc, train_accuracies = get_best_hyperparameters(
        X_train_scaled, y_train, max_depth, alphas, betas
    )
...
def plot_heatmap(mean_train_accuracies, alphas, betas, dataset_name):
    ...
    # Save the heatmap
    plt.savefig(f"figures/{dataset_name}_heatmap.pdf")
    tikzplotlib.save(f"figures/{dataset_name}_heatmap.tex")
    plt.close()


def experiment_on_datasets_single_tree(
    seeds, max_depth=5, alphas=np.linspace(1, 10, 10), betas=np.linspace(1, 10, 10)
):
    datasets = [
        (267, "Banknote"),
        # (17, "BCW-D"),
    ...
    for dataset_id, dataset_name in datasets:
        ...
        for seed in seeds:
            best_params, best_train_acc, test_acc, num_nodes, train_accuracies = (
                run_dtree_on_dataset(
                    X, y, max_depth=max_depth, alphas=alphas, betas=betas, seed=seed
                )
            )
            ...
            print(
                f"{dataset_name} {seed} - Best Params: {best_params}, Train Accuracy: {best_train_acc:.4f}, Test Accuracy: {test_acc:.4f}, Num Nodes: {num_nodes}"
            )
        ...
        print(
            f"{dataset_name} - Mean Test Accuracy: {mean_test_accuracy:.4f}, Std: {std_test_accuracy:.4f}, Mean Num Nodes: {mean_num_nodes:.2f}, Std Num Nodes: {std_num_nodes:.2f}"
        )
    return results
...
if __name__ == "__main__":
    ...
    results = experiment_on_datasets_single_tree(
        seeds, max_depth=4, alphas=alphas, betas=betas
    )
    ...
    for dataset, metrics in results.items():
        print(
            f"{dataset} - Mean Test Accuracy: {metrics['mean_test_accuracy']:.4f}, Std: {metrics['std_test_accuracy']:.4f}, Mean Num Nodes: {metrics['mean_num_nodes']:.2f}, Std Num Nodes: {metrics['std_num_nodes']:.2f}"
        )
