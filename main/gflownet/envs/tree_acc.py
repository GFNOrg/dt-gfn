...
class Tree(GFlowNetEnv):
    ...
    def get_mask_invalid_actions_forward(
        ...
        if stage == Stage.COMPLETE:
            ...
            if not self.continuous and len(self.thresholds) == 1:
                k = self.find_active(state)
                parents = []
                while k is not None:
                    parents.append(k)
            ...
            if self.mask_redundant_choices: 
                ...
                path = []  # Store tuples of (threshold, direction)
                current_k = k

                # Traverse from current node to root and collect thresholds with directions
                while current_k != 0:  # Assuming 0 is the root and has no parent
                    parent = Tree._get_parent(current_k)
                    if parent is not None:
                        parent_feature = state[parent, Attribute.FEATURE].item()
                        if parent_feature == feature_chosen:
                            parent_threshold = state[parent, Attribute.THRESHOLD].item()
                            # If current_k is a left child, we're in the "less than" branch
                            # If current_k is a right child, we're in the "greater than or equal" branch
                            is_left = (current_k % 2) == 1
                            path.append((parent_threshold, "left" if is_left else "right"))
                    current_k = parent
                ...
                if path:
                    for idx in range(
                        ...
                        # Check against all constraints in the path
                        for threshold, direction in path:
                            if threshold < 0:  # Handling -1 fillers
                                continue
                            if direction == "left":
                                # In left branch: new threshold must be less than parent threshold
                                if current_threshold >= threshold:
                                    mask[idx] = True
                                    break
                            else:  # direction == "right"
                                # In right branch: new threshold must be greater than parent threshold
                                if current_threshold <= threshold:
                                    mask[idx] = True
                                    break
